...
while 1:
	...
	if symbol_provider.has_key("_init"):
	    debug(DEBUG_SPAM, library, ": Library has a constructor!");
	    using_ctor_dtor = 1
	...
	if symbol_provider.has_key("_fini"):
	    debug(DEBUG_SPAM, library, ": Library has a destructor!");
	    using_ctor_dtor = 1
	...
	try:
        	...
        	if not is_weak:
            	    debug(DEBUG_SPAM, "No library provides non-weak ",symbol)
    	    else:
