...
printf("groesse         : %d\n",size);
        ...
        for(i=0; i< size; i++) input[i]=fgetc(pFile);
        fclose(pFile);
...
if  (no_next_in_group==1) {
		for(i=0;i<4;i++) dirheader0.to_next_dir_file_in_group[i]=0xff;
	      }
else
	      {
...
if (empty==1) {
		if (no_next_in_group==1) {
					  for(i=0;i<4;i++) dirheader0.to_next_dir_file[i]=0x00;
	      				 }
		else
					 {
					 for(i=0;i<4;i++) dirheader0.to_next_dir_file[i]=0xff;
					 }
	      }				
else
	      {
...
for(i=0;i<6;i++)  dirheader0.garbage1[i]=0x00;
dirheader0.ka=0x01;
dirheader0.dir_type=0xed;
for(i=0;i<8;i++)  dirheader0.garbage2[i]=0x00;
...
for(i=0;i<4;i++) fileheader0.to_next_dir_file_in_group[i]=0xff;
...
for(i=0;i<6;i++)  fileheader0.garbage1[i]=0x00;
fileheader0.ka=0x01;
fileheader0.file_type=0xb6;
for(i=0;i<8;i++) fileheader0.garbage2[i]=0x00;
...
for(a=0;a<blockzahl;a++) {
	...
	SWAP_WORD(blockNR);
	if (a<blockzahl-1) { data_block_header0.next_block_block=blockNR; }
	else { data_block_header0.next_block_block=0xffff; }
	SWAP_WORD(blockNR);
	...
	if (a<blockzahl-1) { data_block_header0.typ2=0xfd; }
	else { data_block_header0.typ2=0xff; }

	data_block_header0.datum=0x00;
	for(i=0;i<4;i++) data_block_header0.garbage[i]=0x00;
	...
	if (data<0xffff)
		...
		{
		if (mode==0) { dir_index(data,plen+30,1); }
		else { dir_index(data,plen+30,0); }
		}
...
if (mode==0) 
	{
	for(i=data;i<chksum;i++) sector_00[i]=0xff;
...
else
	{
        for(i=data;i<chksum;i++) sector_00[i]=0xff;
        sector_00[chksum]=0xfb;
        for(i=data_00;i<chksum_00;i++) sector_00[i]=0xff;
